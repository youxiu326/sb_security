...
nohup java  $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -jar $LIB_JARS  --spring.config.location=$CONF_DIR/ > $STDOUT_FILE 2>&1 &
COUNT=0
checkTime=0
while [ $COUNT -lt 1 ]; do    
    echo -e ".\c"
    ...
    if [ $COUNT -gt 0 -o $checkTime -gt 120 ]; then
        break
    fi
